...
class TemporalConvLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.mul((x_p + x_res), self.sigmoid(x_q))#>[32, 64, 12, 67],x_p;x_q;x_res_[32, 64, 12, 67]
    ...
    def forward(self, x):#([32, 16, 10, 228])Ks=3
        ...
        if self.Ks - 1 < 0:
            ...
            for k in range(2, self.Ks):# 2
                x_list.append(torch.einsum('hi,btij->bthj', 2 * self.gso, x_list[k - 1]) - x_list[k - 2])##Chebyshev polynomials
...
class STConvBlock(nn.Module):
    def __init__(self, Kt, Ks, n_vertex, last_block_channel, channels, act_func, graph_conv_type, gso, bias, droprate):
        ...
        self.tmp_conv1 = TemporalConvLayer(Kt, last_block_channel, channels[0], n_vertex, act_func)##Convert 1 to 64

        self.graph_conv = GraphConvLayer(graph_conv_type, 1, channels[1], Ks, gso, bias)#####If you use the sandwich structure, you need to change channels[0] to 1, and change 1 to 16.

        self.tmp_conv2 = TemporalConvLayer(Kt,channels[1], channels[2], n_vertex, act_func)###Convert 16 to 64
        
        self.graph_conv2 = GraphConvLayer(graph_conv_type,64,64, Ks, gso, bias)##If you use the sandwich structure, you need to change channels[0] to 1
